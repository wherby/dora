...
object ProcessServiceSpec {
  def processServiceNameToClassOpt(
      className: String,
  ...
  def safeProcessServiceNameToClassOpt(
      className: String,
      classLoaderOpt: Option[ClassLoader]
  ): Option[Class[_]] = {
...
class ProcessServiceSpec extends FlatSpec with Matchers {
  ...
  val processCallMsg = ProcessCallMsg(
    "doracore.util.TestProcessor",
    "addPar",
    Array(Par1(2).asInstanceOf[AnyRef], Par2(4).asInstanceOf[AnyRef])
  )
  ...
  "Process service" should "return result for Command sercice call in object" in {
    ProcessService.nameToClassOpt = ProcessServiceSpec.processServiceNameToClassOpt
    val msg =
      processCallMsg.copy(clazzName = "doracore.util.TestProcessor2", methodName = "objectAdd")
    val result = ProcessService.callProcess(msg)
    result shouldBe (Right(Par1(6)))
  }
  ...
  "Process Service " should "return left  in SimpleProcessFuture use with wrong parameters" in {
    ProcessService.nameToClassOpt = ProcessServiceSpec.processServiceNameToClassOpt
    val msg = processCallMsg.copy(
      clazzName = "doracore.util.TestProcessor",
      methodName = "addFuture",
      paras = Array(2.asInstanceOf[AnyRef], 4.asInstanceOf[AnyRef])
    )
  ...
  "Process Service " should "return futureResult in SimpleProcessFuture use" in {
    ProcessService.nameToClassOpt = ProcessServiceSpec.processServiceNameToClassOpt
    val msg =
      processCallMsg.copy(clazzName = "doracore.util.TestProcessor", methodName = "addFuture")
    val result = ProcessService.callProcessAwaitFuture(msg)
  ...
  "Process Service " should "return futureResult with failed  in callProcessFutureResult use when use wrong classname " in {
    val msg =
      processCallMsg.copy(clazzName = "doracore.util.TestProcesso3", methodName = "addFuture")
    val result = Await.result(ProcessService.callProcessFutureResult(msg), ConstVars.timeout1S)
  ...
  "Get Future result" should "return right when result is  Future" in {
    val result = ProcessService.getFutureResult(
      Future("result")(scala.concurrent.ExecutionContext.Implicits.global),
      3600 seconds
    )
