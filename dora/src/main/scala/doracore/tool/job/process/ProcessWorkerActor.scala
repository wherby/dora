...
class ProcessWorkerActor extends WorkerActor with ActorLogging {
  ...
  def handleSimpleProcessFutureInit(simpleProcessFutureInit: SimpleProcessFutureInit) = {
    futureResultOpt = Some(
      ProcessService.callProcessFutureResult(simpleProcessFutureInit.processCallMsg)
    )
  ...
  override def receive: Receive = super.receive orElse {
    ...
    case simpleProcessFutureInit: SimpleProcessFutureInit =>
      handleSimpleProcessFutureInit(simpleProcessFutureInit)
